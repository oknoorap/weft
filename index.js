...
class Weft {
  ...
  view(fontName, fields = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.list(defaultFields)
        .then((result) => {
          const filteredFont = result.filter(
          ...
          const searchCriteria = result.filter(({ item }) => {
            ...
            if (_options.subsets) {
              ...
              for (const key in _options.subsets) {
                if (
                  Object.prototype.hasOwnProperty.call(_options.subsets, key) &&
  ...
  embedUrl(fontName, options = {}, subsets = "") {
    ...
    if (!skip && _options.variants) {
      ...
      for (const key in _options.variants) {
        if (
          Object.prototype.hasOwnProperty.call(_options.variants, key) &&
